...
- Configured key to enable number layer with mod (GUI). Used for custom AwesomeWM bindings.
- Added numbers layer persistent toggle.
...
- 2020/12/28
  Reworked number layer, added number layer peristent toggle to keep the layer active without pressing constantly a button. Tried to exclude some keys to emulate common Dactyl Manuform layouts. Added new layout to enable ~_NUMB~ layer with ~GUI~ modifier enabled for switching AwesomeWM tag.
